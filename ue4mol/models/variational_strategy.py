...
class FixedIndPoint_VariationalStrategy(_VariationalStrategy):
    r"""
    ...
    def __init__(self, model, inducing_points, variational_distribution, learn_inducing_locations=True, device=torch.device("cuda")):
        super().__init__(model, inducing_points, variational_distribution, learn_inducing_locations)
        self.register_buffer("updated_strategy", torch.tensor(True))
        self._register_load_state_dict_pre_hook(_ensure_updated_strategy_flag_set)
        self.fixed_ind_point = torch.zeros(1, inducing_points.shape[1], requires_grad=False).to(device)
    ...
    def forward(self, x, inducing_points, inducing_values, variational_inducing_covar=None, **kwargs):
        ...
        full_inputs = torch.cat([self.fixed_ind_point, inducing_points, x], dim=-2)
        #full_inputs = torch.cat([inducing_points, x], dim=-2)
        full_output = self.model.forward(full_inputs, **kwargs)
        full_covar = full_output.lazy_covariance_matrix

        # Covariance terms
        num_induc = inducing_points.size(-2) + 1
        ...
        #predictive_mean = (interp_term.transpose(-1, -2) @ torch.cat((torch.zeros(1).cuda(), inducing_values)).unsqueeze(-1)).squeeze(-1) + test_mean
    ...
    def __call__(self, x, prior=False, **kwargs):
        if not self.updated_strategy.item() and not prior:
            with torch.no_grad():
                # Get unwhitened p(u)
                prior_function_dist = self(torch.cat([self.fixed_ind_point, self.inducing_points]), prior=True)
